...
class RejectPayoutRequest extends Notification {
    use Queueable;
    ...
    public function __construct($referralPayout) {
        ...
        $this->template       = EmailTemplate::where('slug', 'REFERRAL_PAYOUT_REJECTED')->first();
        Overrider::load('Settings');

        $this->replace['name']            = $this->referralPayout->user->name;
        $this->replace['amount']          = decimalPlace($this->referralPayout->amount, currency());
        $this->replace['requestDate']     = $this->referralPayout->created_at;
        $this->replace['rejectionReason'] = $this->referralPayout->admin_note;
    }
