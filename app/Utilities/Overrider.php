...
class Overrider {
    ...
    public static function loadBusinessSettings($businessId = '') {
        $email_protocol = get_business_option('mail_type', '', $businessId);
        ...
        if ($email_protocol == '') {
            ...
            config(['mail.from.name' => get_business_option('from_name', '', $businessId)]);
            config(['mail.from.address' => get_business_option('from_email', '', $businessId)]);

            if ($email_protocol == 'smtp') {
                config(['mail.mailers.smtp.host' => get_business_option('smtp_host', '', $businessId)]);
                config(['mail.mailers.smtp.port' => get_business_option('smtp_port', '', $businessId)]);
                config(['mail.mailers.smtp.username' => get_business_option('smtp_username', '', $businessId)]);
                config(['mail.mailers.smtp.password' => get_business_option('smtp_password', '', $businessId)]);
                config(['mail.mailers.smtp.encryption' => get_business_option('smtp_encryption', '', $businessId)]);
            }
        }
    }
