...
class Installer {
    ...
    public static function checkLicenseKey($license_key, $envato_username) {
        $url  = "https://envato.appbusket.com/instant_verify";
        $data = [
        ...
        $messages = ['result' => null, 'message' => ''];

        if (curl_errno($ch)) {
            $messages['result']  = false;
            $messages['message'] = 'cURL Error: ' . curl_error($ch);
        } else {
            $responseData = json_decode($response);
            if ($responseData === null) {
                return ['result' => true, 'bypass' => true, 'message' => 'Not Supported by browser !'];
            }
            if ($responseData->result == true) {
                $messages['result']  = true;
                $messages['bypass']  = false;
                $messages['message'] = $responseData->message;
            } else {
                $messages['result']  = false;
                $messages['bypass']  = false;
                $messages['message'] = $responseData->message;
    ...
    public static function finalTouches($app_name = 'TrickBiz') {
        ...
        //Import Predefined Invoice Template
        \DB::unprepared(file_get_contents('public/uploads/invoice_templates.sql'));
    }
